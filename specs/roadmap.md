...
6. **Integrar API do Fogo Cruzado (violência armada, Belém desde nov/2023)** — bloqueado esperando o usuário criar conta em `fogocruzado.org.br` e passar a chave de API (agente não cria contas de terceiro). Assim que a chave chegar: schema novo (`armed_violence_events`, evento pontual — não reaproveita `incident_patterns`), endpoint API novo, camada visual separada no mapa. Ver [data-sources.md](data-sources.md).
7. **Pedido formal ao MPPA (CAO Criminal, `caocriminal@mppa.mp.br`)** — canal paralelo ao LAI da SEGUP-PA, pedindo dado por bairro/delegacia em Belém. Ainda não redigido/enviado.
